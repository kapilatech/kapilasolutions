...
_g.shared.HTTP = new function() {
    ...
    return {
        ...
        addSelector: function(url, selector, index) {
            ...
            var sIndex = url.lastIndexOf("index.html");
        ...
        getSelectors: function(url) {
            ...
            var fragment = url.substring(url.lastIndexOf("index.html"));
            if (fragment) {
...
_g.shared.Sling = function() {
    ...
    return {
        ...
        getContentPath: function(relPath, absPath, allowParentPaths) {
            var path = absPath;
            if (path.lastIndexOf(".") > path.lastIndexOf("index.html")) {
                // remove selectors and extension from absPath:
                // /content/foo.bar.html >> /content/foo
                path = path.substr(0, path.indexOf(".", path.lastIndexOf("index.html")));
            }
            if (relPath) {
                if (relPath.indexOf("index.html") == 0) {
                    path = relPath;
                } else {
                    if (allowParentPaths) {
                        while (relPath.indexOf("index.html") == 0) {
